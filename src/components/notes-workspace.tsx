...
export function NotesWorkspace({
  ...
  // 文件夹栏自己的收起开关（与导航栏无关）：收起只留一条窄边放箭头，中间「全部笔记栏」始终常驻。
    ...
    <div className="relative flex md:h-screen">
      ...
      {!collapsed && (
        <aside
          ...
          className="relative hidden shrink-0 flex-col border-r border-zinc-200 bg-zinc-50 md:flex print:hidden"
        >
      ...
      {/* ===== 中：笔记列表栏（桌面，可拖宽；常驻不随收起消失） ===== */}
      <div
        style={{ width: listResize.width }}
        className="relative hidden shrink-0 flex-col border-r border-zinc-200 bg-white md:flex print:hidden"
      >
        <div className="flex items-center gap-0.5 border-b border-zinc-200 px-2 py-2">
      ...
      {/* ===== 右：内容栏 ===== */}
      <div className="min-w-0 flex-1 md:overflow-y-auto print:overflow-visible">{children}</div>
    </div>
  );
}
