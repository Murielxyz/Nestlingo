...
import { parseNote } from "@/lib/parse-sections";
...
/** 学伴「语言」下拉：默认自由对话；选了目标语言则进入翻译/场景/美文模式（通用，不只为一个目标服务）。 */
const LANGS = [
  { key: "", label: "自由" },
  { key: "ko", label: "韩语" },
  { key: "th", label: "泰语" },
...
/** 从 localStorage 读学伴历史（按笔记隔离，重开面板还能看到之前的对话）。 */
...
/** 从当前笔记里抽出已有知识点（生词/例句/语法，正面 + 现有释义），供学伴「补全」作上下文。
...
function notePointsOf(
  ...
  if (!editor) return [];
  try {
    const { sections } = parseNote(docToText(editor.getJSON()));
    const out: { kind: "word" | "example" | "grammar"; front: string; back: string }[] = [];
    for (const s of sections) {
      for (const c of s.cards) {
        if (!c.front || alreadyEnriched(c.back)) continue;
        out.push({ kind: s.kind, front: c.front, back: c.back });
      }
    }
    return out;
  } catch {
...
export function AiAssistantPanel({
  ...
  // 多合一输入：extracting = 正在把 图片/语音/链接 转文字；recording = 正在录音。
  ...
  function addToNote() {
    ...
    editor.chain().focus("end").insertContent(pointsToNoteContent(pts)).run();
    setDone(`已把 ${pts.length} 个知识点加到笔记末尾`);
  }
  ...
  /** 把学伴返回的富化点（生词/例句/语法）覆盖写回笔记对应的 callout：
   *  富化的替换薄的那条，其余已富化/未命中的原样保留（按 front 去重，不重复、不丢）。 */
  function replaceOriginal() {
    if (!editor) return;
    const pts = selectedPoints().filter((p) => p.kind !== "article");
    if (pts.length === 0) return;

    // 现有笔记里的知识点（含已富化的），按 front 归并。
    const existing: { kind: "word" | "example" | "grammar"; front: string; back: string }[] = [];
    try {
      const { sections } = parseNote(docToText(editor.getJSON()));
      for (const s of sections) {
        for (const c of s.cards) {
          if (!c.front) continue;
          existing.push({ kind: s.kind, front: c.front, back: c.back });
        }
      }
    } catch {
      /* 解析失败则只写回富化点 */
    ...
    const enrichedFronts = new Set(pts.map((p) => p.front));
    const merged: AssistantPoint[] = [...pts];
    for (const e of existing) {
      if (enrichedFronts.has(e.front)) continue; // 已富化，跳过原条目
      merged.push({
        kind: e.kind,
        front: e.front,
        back: e.back.replace(/\n/g, "  "), // 换行回写成双空格，转成闪卡时再拆回
    ...
    const blocks = pointsToNoteContent(merged);
    ...
    for (const node of content) {
      if (
        node.type === "callout" &&
        (node.attrs?.kind === "word" ||
          node.attrs?.kind === "example" ||
          node.attrs?.kind === "grammar")
      ) {
        const kind = node.attrs.kind as string;
        const block = blocks.find((b) => b.type === "callout" && b.attrs?.kind === kind);
        if (block && !placed.has(kind)) {
          next.push(block);
          placed.add(kind);
        }
        // 其余同类 callout 丢弃（并入上面那一个）
      } else {
        next.push(node);
      }
    }
    for (const block of blocks) {
      const kind = block.attrs?.kind as string;
      if (kind && !placed.has(kind)) next.push(block);
    }
    editor.commands.setContent({ type: "doc", content: next });
    setDone(`已覆盖回笔记（富化 ${pts.length} 项，其余原样保留）`);
  }
  ...
  async function transcribeAudio(blob: Blob) {
    ...
    try {
      ...
      // 语音识别后自动发送、继续对话（口语练习一句接一句）；图片/链接仍是转文字填框、编辑后发。
      await sendText(text);
      setDone("已识别语音并发送");
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  /** 链接：贴 YouTube / 音频 / 播客链接 → 抓文字稿填进输入框。 */
  async function fetchLink() {
    const url = linkUrl.trim();
    if (!url || extracting) return;
    if (YTDLP_DISABLED && parseMediaUrl(url)?.kind === "youtube") {
      setError("此功能暂未启用");
      return;
    }
    setError(null);
    setDone(null);
    setExtracting("正在抓取链接文字稿…");
    try {
      ...
      <div className="fixed bottom-0 right-0 top-0 z-50 flex w-full max-w-[420px] flex-col bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-semibold text-zinc-900">AI 学伴</p>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="rounded-md px-2 py-1.5 text-xs text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700"
            >
              清除会话
            </button>
            <button
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
              ...
              <div className="mt-2.5 flex gap-2">
                <button
                  ...
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-teal-200 px-3 py-1.5 text-sm font-medium text-teal-700 transition-colors hover:bg-teal-50 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-teal-600 px-3 py-1.5 text-sm font-semibold text-white transition-colors hover:bg-teal-700 disabled:opacity-50"
                >
                  {addingCards ? (
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  ) : (
              ...
              {selectedPoints().some((p) => p.kind !== "article") && (
                <button
                  onClick={replaceOriginal}
                  className="mt-1.5 w-full rounded-lg border border-violet-200 px-3 py-1.5 text-xs font-medium text-violet-600 transition-colors hover:bg-violet-50"
                >
                  覆盖原始笔记
                </button>
              )}
        ...
        <div className="border-t border-zinc-100 p-3">
          ...
          {linkOpen && (
            <div className="mb-2 flex items-center gap-2">
              <input
                ...
                placeholder="粘贴 YouTube / 音频 / 播客链接"
                autoFocus
                className="min-w-0 flex-1 rounded-lg border border-zinc-200 px-3 py-1.5 text-sm text-zinc-800 focus:border-teal-500 focus:outline-none"
              />
              <button
          ...
          <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
            ...
            <button
              ...
              className="rounded-full border border-teal-200 px-2.5 py-0.5 text-[11px] text-teal-600 transition-colors hover:bg-teal-50"
            >
              翻译
            </button>
          </div>
          ...
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={2}
              className="min-h-0 flex-1 resize-none rounded-xl border border-zinc-200 px-3 py-2 text-sm text-zinc-800 focus:border-teal-500 focus:outline-none"
            />
            <button
              onClick={() => void send()}
          ...
          <div className="mt-1.5 flex items-center gap-0.5">
            ...
            <button
              ...
              title="粘贴链接转文字稿"
              aria-label="粘贴链接"
              className={`${TOOL_BTN} ${linkOpen ? "bg-zinc-100 text-teal-600" : ""}`}
            >
