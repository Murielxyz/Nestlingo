  ...
  if (mode === "test") {
    ...
    try {
      if (theme) {
        ...
        title = noteId ? ((await getNote(noteId))?.title ?? "测试") : "独立闪卡";
  ...
  if (mode === "cloze") {
    ...
    try {
      if (theme) {
        ...
        title = noteId ? ((await getNote(noteId))?.title ?? null) : "独立闪卡";
        if (cardKind) title += kindSuffixOf(cardKind);
      }
    } catch (err) {
      error = friendlyQueryError(err);
      ...
      <div>
        ...
        {error ? (
          ...
          <EmptyState
            icon={<Activity className="h-10 w-10" />}
            title="请先选择合集"
            description="回复习主页，从「正在背的合集」点「完形填空」进入。"
          />
        ) : (
          <ClozeSession
    ...
    return (
      <div>
        ...
        {error ? (
          ...
          <ReviewSession items={items} dailyGoal={dailyGoal} canRememberCollection={false} />
        )}
      </div>
    );
  }
